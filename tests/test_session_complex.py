...
class SessionComplexTests(unittest.TestCase):
    ...
    def test_multiple_solves_accumulate_context(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            state_path = root / ".openscout" / "sessions" / "accum" / "state.json"
    ...
    def test_observation_trimming_on_persist(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            state_path = (
                root / ".openscout" / "sessions" / "trim-test" / "state.json"
            )
            state = json.loads(state_path.read_text(encoding="utf-8"))
    ...
    def test_event_log_integrity(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            events_path = (
                root / ".openscout" / "sessions" / "evlog" / "events.jsonl"
            )
    ...
    def test_step_callback_patch_format(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            events_path = (
                root / ".openscout" / "sessions" / "patch-fmt" / "events.jsonl"
            )
            events = []
            for line in events_path.read_text(encoding="utf-8").splitlines():
                line = line.strip()
